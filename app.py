...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ATTENDANCE â€” with lock + monthly % feature
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    for rec in d.get("records", []):
        ...
        if rec["status"] in ("A", "Absent"):
            msg = (f"Dear {st.parent_name},\n\nAapka bachha *{st.name}* (Roll {st.roll_no}) "
                  ...
                  f"ðŸ« {school.name}, {school.city}\n\nKoi karan ho to school inform karein. ðŸ™")
...
def add_homework():
    ...
    wa_sent = 0
    for st in students:
        due_str = hw.due_date.strftime('%d %b %Y') if hw.due_date else "Kal tak"
        msg = (f"Dear {st.parent_name},\n\nðŸ“š *Homework Alert*\n"
              f"Student: {st.name} (Roll {st.roll_no})\nSubject: {hw.subject}\n"
              f"Title: *{hw.title}*\nDetails: {hw.description}\nDue: {due_str}\n\nðŸ« {school.name} ðŸ™")
        send_wa(school, st.parent_phone, msg, "homework"); wa_sent += 1

    hw.wa_sent = True
    db.session.commit()

    send_to_make("homework_assigned", {
        "school": school.name, "class_id": d["class_id"], "subject": hw.subject,
        ...
        "message": (f"ðŸ“š Homework Alert\nSubject: {hw.subject}\nTitle: {hw.title}\n"
                    f"Details: {hw.description}\nDue: {hw.due_date.strftime('%d %b %Y') if hw.due_date else 'Kal tak'}\n"
                    f"ðŸ« {school.name}"),
        "parents": [
            {"student_name": st.name, "roll_no": st.roll_no,
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# ðŸ¢ SUPER ADMIN â€” manage all schools
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
if __name__ == "__main__":
    with app.app_context():
        ...
        if School.query.count() == 0:
            ...
            db.session.commit()
            print("âœ… Demo schools created!")
        print("âœ… Database ready!")
        print(f"ðŸ”‘ Super Admin login â†’ username: {SUPER_ADMIN_USER} / password: {SUPER_ADMIN_PASS}")
