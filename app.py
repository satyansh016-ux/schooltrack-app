...
import requests, os, calendar, json
...
# NOTE: Render's free tier blocks outbound SMTP ports (25/465/587), so we
# send email over HTTPS via Brevo's transactional email API instead of SMTP.
BREVO_API_KEY     = os.environ.get("BREVO_API_KEY", "")
BREVO_SENDER_EMAIL = os.environ.get("BREVO_SENDER_EMAIL", "")
ABSENT_ALERT_THRESHOLD = int(os.environ.get("ABSENT_ALERT_THRESHOLD", 8))
RENEWAL_REMINDER_DAYS = int(os.environ.get("RENEWAL_REMINDER_DAYS", 5))
CRON_SECRET = os.environ.get("CRON_SECRET", "change_this_secret")

# ── Web Push Notifications (Homework, Notices, Attendance) ──
VAPID_PUBLIC_KEY  = os.environ.get("VAPID_PUBLIC_KEY", "")
VAPID_PRIVATE_KEY = os.environ.get("VAPID_PRIVATE_KEY", "")
VAPID_CLAIM_EMAIL = os.environ.get("VAPID_CLAIM_EMAIL", "mailto:admin@schooltrack.app")


def send_push_to_student(student_id, title, body, url="/parent"):
    """Send a browser push notification to every device a parent enabled
    notifications on for this student. Silently skips if VAPID not configured."""
    ...
    subs = PushSubscription.query.filter_by(student_id=student_id).all()
    for sub in subs:
        try:
            webpush(
                subscription_info={
...
def send_email(to_email, subject, body):
    """Send an email over HTTPS using Brevo's API (works on Render free tier,
    unlike raw SMTP which is blocked on free web services)."""
    if not (BREVO_API_KEY and BREVO_SENDER_EMAIL and to_email):
        return False
    try:
        r = requests.post(
            ...
            json={
                "sender": {"email": BREVO_SENDER_EMAIL, "name": "SchoolTrack"},
                "to": [{"email": to_email}],
                "subject": subject,
                "textContent": body
            },
...
def send_renewal_reminder_email(school, days_left):
    if days_left <= 0:
        subject = f"🔴 SchoolTrack subscription EXPIRED — {school.name}"
        urgency = "Aapka subscription AAJ ya usse pehle expire ho chuka hai."
    else:
        subject = f"⏳ SchoolTrack subscription {days_left} din mein expire hoga — {school.name}"
        urgency = f"Aapka subscription sirf *{days_left} din* mein expire ho jayega."

    body = (
        f"Namaste,\n\n{urgency}\n\n"
        f"School: {school.name}\n"
        f"Expiry Date: {school.expiry_date.strftime('%d %b %Y')}\n"
        f"Plan: {school.plan}\n\n"
        f"Service continue rakhne ke liye kripya jald hi renew/payment kar dein, "
        f"warna is date ke baad school aur parent portal ka access band ho jayega.\n\n"
        f"— SchoolTrack (Automated Reminder)"
    )
    return send_email(school.principal_email, subject, body)
...
class School(db.Model):
    ...
    wa_absent_enabled = db.Column(db.Boolean, default=True)
...
class Notice(db.Model):
    """School-wide events, holidays, fee reminders, exam timetables, results,
    and emergency alerts — all unified under one model with a category.
    Shown in the Parent Portal. Independent of WhatsApp/Make.com."""
    id          = db.Column(db.Integer, primary_key=True)
    school_id   = db.Column(db.Integer, db.ForeignKey("school.id"), nullable=False)
    title       = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text)
    event_date  = db.Column(db.Date)   # optional — null for general notices
    category    = db.Column(db.String(20), default="general")  # general/fee/exam/result/emergency
    amount      = db.Column(db.Float)  # used only for category="fee"
    created_at  = db.Column(db.DateTime, default=datetime.utcnow)


class PushSubscription(db.Model):
    """A parent's browser push subscription (one row per device they enabled
    notifications on), used to send Homework/Notice/Attendance push alerts."""
    id         = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey("student.id"), nullable=False)
    endpoint   = db.Column(db.Text, nullable=False)
    p256dh     = db.Column(db.String(300))
    auth       = db.Column(db.String(300))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def login_required(f):
    ...
    def decorated(*a, **kw):
        ...
        if s.expiry_date < date.today():
            return jsonify({"error": "Subscription expired. Contact your service provider to renew.",
                            "expired": True}), 403
        return f(*a, **kw)
    return decorated
...
def login():
    ...
    if school.expiry_date < date.today():
        return jsonify({"error": "Subscription expired. Contact your service provider to renew.",
                        "expired": True}), 403
...
def mark_attendance():
    ...
    for rec in d.get("records", []):
        ...
        if rec["status"] in ("A", "Absent"):
            send_push_to_student(st.id, f"📅 {school.name}: Attendance Alert",
                                 f"{st.name} marked ABSENT today ({att_date.strftime('%d %b %Y')})")
            if school.wa_absent_enabled:
            ...
            if absent_count >= ABSENT_ALERT_THRESHOLD:
                ...
                already_sent = WaLog.query.filter_by(
                    school_id=school.id, msg_type="principal_alert_marker",
                    phone=marker, status="sent"
                ).first()
                if not already_sent:
                    ok = send_principal_email(school, st, absent_count)
...
def add_homework():
    ...
    push_body = f"{hw.subject}: {hw.description[:80]}" if hw.description else hw.subject
    for st in students:
        send_push_to_student(st.id, f"📚 {school.name}: {hw.title}", push_body)
...
def wa_log():
    ...
    logs = WaLog.query.filter_by(school_id=school.id)\
                      .filter(WaLog.msg_type.notin_(["principal_alert_marker", "renewal_reminder_marker"]))\
...
def parent_required(f):
    ...
    def wrapper(*a, **kw):
        ...
        school = School.query.get(session.get("parent_school_id"))
        if not school or school.status == "suspended" or school.expiry_date < date.today():
            session.pop("parent_student_id", None)
            session.pop("parent_school_id", None)
            return jsonify({"error": "This school's SchoolTrack subscription is inactive."}), 403
        return f(*a, **kw)
    return wrapper
...
def parent_portal_page():
    return render_template("parent.html", manifest_url="/static/parent-manifest.json", prefill_school="")


@app.route("/parent/<school_username>")
def parent_portal_page_branded(school_username):
    school = School.query.filter_by(username=school_username).first()
    if not school:
        return render_template("parent.html", manifest_url="/static/parent-manifest.json", prefill_school="")
    return render_template(
        "parent.html",
        manifest_url=f"/manifest/parent/{school_username}.json",
        prefill_school=school_username
    )


@app.route("/manifest/parent/<school_username>.json")
def parent_manifest_branded(school_username):
    school = School.query.filter_by(username=school_username).first()
    name = f"{school.name} — Parent Portal" if school else "Parents Portal"
    short_name = (school.name[:12] if school else "Parents")
    return jsonify({
        "name": name,
        "short_name": short_name,
        "description": "Stay Connected, Stay Involved",
        "start_url": f"/parent/{school_username}",
        "display": "standalone",
        "background_color": "#00BB94",
        "theme_color": "#00BB94",
        "orientation": "portrait",
        "icons": [
            {"src": "/static/parent-icon-192.png", "sizes": "192x192", "type": "image/png"},
            {"src": "/static/parent-icon-512.png", "sizes": "512x512", "type": "image/png"}
        ]
    })
...
# ── School-side (teacher/principal) Notice management ──
@app.route("/api/notices", methods=["GET"])
@login_required
def list_notices():
    school = get_school()
    rows = Notice.query.filter_by(school_id=school.id)\
                       .order_by(Notice.event_date.desc().nullslast(), Notice.created_at.desc()).all()
    return jsonify([{
        "id": n.id, "title": n.title, "description": n.description,
        "event_date": n.event_date.strftime("%d %b %Y") if n.event_date else None,
...
def add_notice():
    ...
    if category not in ("general", "fee", "exam", "result", "emergency"):
        category = "general"
    n = Notice(
        school_id=school.id, title=d["title"].strip(),
    ...
    icon = {"general": "📢", "fee": "💰", "exam": "📝", "result": "🏆", "emergency": "🚨"}.get(category, "📢")
    push_title = f"{icon} {school.name}: {n.title}"
    push_body = n.description[:100] if n.description else "Tap to view details"
    for st in Student.query.filter_by(school_id=school.id).all():
        send_push_to_student(st.id, push_title, push_body)

    return jsonify({"msg": "Notice added", "id": n.id}), 201
...
def delete_notice(nid):
    school = get_school()
    n = Notice.query.filter_by(id=nid, school_id=school.id).first()
    if not n:
...
# ── Parent-side Notice viewing ──
@app.route("/api/parent/notices", methods=["GET"])
@parent_required
def parent_notices():
    student = Student.query.get(session["parent_student_id"])
    if not student:
        return jsonify({"error": "Student not found"}), 404
    rows = Notice.query.filter_by(school_id=student.school_id)\
                       .order_by(Notice.event_date.desc().nullslast(), Notice.created_at.desc())\
                       .limit(30).all()
    return jsonify([{
        "title": n.title, "description": n.description,
        "event_date": n.event_date.strftime("%d %b %Y") if n.event_date else None,
        "posted_on": n.created_at.strftime("%d %b %Y"),
        "category": n.category or "general",
        "amount": n.amount
    } for n in rows])


@app.route("/api/parent/vapid-public-key", methods=["GET"])
def parent_vapid_public_key():
...
def parent_login():
    ...
    if school.status == "suspended" or school.expiry_date < date.today():
        return jsonify({"error": "This school's SchoolTrack subscription is inactive. Contact the school."}), 403
...
@app.route("/api/cron/expiry-check", methods=["GET", "POST"])
def cron_expiry_check():
    """Called once a day by an external scheduler (e.g. Make.com's Schedule
    module) to email schools whose subscription expires within
    RENEWAL_REMINDER_DAYS. Protected by a shared secret, not by login."""
    key = request.args.get("key", "")
    if key != CRON_SECRET:
        return jsonify({"error": "Unauthorized"}), 401

    today = date.today()
    cutoff = today + timedelta(days=RENEWAL_REMINDER_DAYS)
    schools = School.query.filter(
        School.status != "suspended",
        School.expiry_date <= cutoff
    ).all()

    sent, skipped = [], []
    for school in schools:
        days_left = (school.expiry_date - today).days
        marker = f"{school.id}:{today.isoformat()}"
        already = WaLog.query.filter_by(
            school_id=school.id, msg_type="renewal_reminder_marker", phone=marker
        ).first()
        if already:
            skipped.append(school.name)
            continue
        ok = send_renewal_reminder_email(school, days_left)
        db.session.add(WaLog(
            school_id=school.id, phone=marker, msg_type="renewal_reminder_marker",
            message=f"{days_left} din baaki (expiry {school.expiry_date})",
            status="sent" if ok else "error"
        ))
        db.session.commit()
        if ok:
    ...
    return jsonify({"date": str(today), "reminders_sent": sent, "already_sent_today": skipped})
...
def admin_list_schools():
    ...
    for s in schools:
        ...
        result.append({
            ...
            "wa_absent_enabled": s.wa_absent_enabled if s.wa_absent_enabled is not None else True,
...
def admin_update_school(sid):
    ...
    if "wa_absent_enabled" in d: s.wa_absent_enabled = bool(d["wa_absent_enabled"])
    db.session.commit()
...
if __name__ == "__main__":
    with app.app_context():
        ...
        try:
            ...
            db.session.rollback()
        try:
            db.session.execute(db.text("ALTER TABLE school ADD COLUMN wa_absent_enabled BOOLEAN DEFAULT TRUE"))
            db.session.commit()
            print("✅ Migrated: added wa_absent_enabled column")
        except Exception:
            db.session.rollback()
        try:
            db.session.execute(db.text("ALTER TABLE notice ADD COLUMN category VARCHAR(20) DEFAULT 'general'"))
            db.session.commit()
            print("✅ Migrated: added notice.category column")
        except Exception:
            db.session.rollback()
        try:
            db.session.execute(db.text("ALTER TABLE notice ADD COLUMN amount FLOAT"))
            db.session.commit()
            print("✅ Migrated: added notice.amount column")
        except Exception:
            db.session.rollback()
